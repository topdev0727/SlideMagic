...
public class VideoGenerator: NSObject {
   ...
   */
    open func generate(withImages _images: [UIImage], secPerImage: Double, andAudios _audios: [URL], andType _type: VideoGeneratorType, _ progress: @escaping ((Progress) -> Void), success: @escaping ((URL) -> Void), failure: @escaping ((Error) -> Void)) {
    
    VideoGenerator.current.setup(withImages: _images, andAudios: _audios, andType: _type)
    
    VideoGenerator.current.audioDurations.removeAll()
    VideoGenerator.current.audioDurations.append(Double(Double(_images.count) * secPerImage))
          ...
          videoWriterInput.requestMediaDataWhenReady(on: media_queue, using: { () -> Void in
            ...
            videoWriter.finishWriting { () -> Void in
              if self.audioURLs.isEmpty {
                let savePathUrl : NSURL = NSURL(fileURLWithPath: NSHomeDirectory() + "/Documents/\(self.fileName).mp4")
                let newPath = savePathUrl as URL
  ...
  fileprivate func setup(withImages _images: [UIImage], andAudios _audios: [URL], andType _type: VideoGeneratorType) {
    ...
    open func filterWithImage(ciImage: CIImage, filter: FilterType) -> CIImage {
        let ciSourceImage = ciImage
        ...
        case .CIPhotoEffectNoir:
            ...
            break;
        case .CIBloom:
            ciFilter = CIFilter(name: "CIBloom")!
            ciFilter!.setValue(ciSourceImage, forKey: kCIInputImageKey)
            ciFilter!.setValue(10.00, forKey: kCIInputRadiusKey)
            ciFilter!.setValue(1.00, forKey: kCIInputIntensityKey)
            break;
        case .CIPhotoEffectProcess:
            ...
            break;
        case .CIPixellate:
            ciFilter = CIFilter(name: "CIPixellate")!
            ciFilter!.setValue(ciSourceImage, forKey: kCIInputImageKey)
        ...
        case .CIPhotoEffectInstant:
            ...
            break;
        case .CILinearToSRGBToneCurve:
            ciFilter = CIFilter(name: "CILinearToSRGBToneCurve")!
            ciFilter!.setValue(ciSourceImage, forKey: kCIInputImageKey)
            break;
        case .CIVibrance:
            ciFilter = CIFilter(name: "CIVibrance")!
            ciFilter!.setValue(ciSourceImage, forKey: kCIInputImageKey)
            ciFilter!.setValue(2.5, forKey: "inputAmount")
            break;
        case .CIPhotoEffectFade:
        ...
        case .CIPhotoEffectMono:
            ...
            break;
        case .CIVignette:
            ciFilter = CIFilter(name: "CIVignette")!
            ciFilter!.setValue(ciSourceImage, forKey: kCIInputImageKey)
            ciFilter!.setValue(1.9, forKey: kCIInputRadiusKey)
            ciFilter!.setValue(1.4, forKey: kCIInputIntensityKey)
            break;
        default:
            break;
        }
        ...
        return outputImage ?? ciSourceImage;
    }
    
    private func ciFilter(withType: FilterType) -> CIFilter? {
        var ciFilter: CIFilter?
    ...
    open func filterVideo(withVideoURL videoUrl: URL, filter: FilterType, success: @escaping ((URL) -> Void), failure: @escaping ((Error) -> Void)) {
        
        if let videoFilter = ciFilter(withType: filter) {
            ...
            let composition = AVVideoComposition(asset: videoAsset, applyingCIFiltersWithHandler: { request in
                ...
                videoFilter.setValue(source, forKey: kCIInputImageKey)
                
                // Crop the blurred output to the bounds of the original image
                let output = videoFilter.outputImage!.cropped(to: request.sourceImage.extent)
            ...
            /// create a path to the video file
            let savePathUrl : NSURL = NSURL(fileURLWithPath: NSHomeDirectory() + "/Documents/Final.mp4")
            let videoOutputURL = savePathUrl as URL
      ...
      for (index, audioUrl) in audioURLs.enumerated() {
        ...
        if shouldAddAudioTrack {
          if let audioTrack = audioAsset.tracks(withMediaType: .audio).first {
            do {
              try audioCompositon?.insertTimeRange(videoTimeRange, of: audioTrack, at: CMTime.zero)
            } catch {
              failure(error)
            }
          }
